...
function TiltBox({ children, className, hoverGrow }: { children: React.ReactNode, className?: string, hoverGrow?: boolean }) {
  ...
  return (
    <div ref={ref} className={twMerge(`${hoverGrow && "hover:scale-105 duration-300"}`, className)}>
      <div ref={ref}
        style={{ transformStyle: "preserve-3d", transform: `perspective(5000px) rotateY(${xTilt}deg) rotateX(${-1 * yTilt}deg)` }}
      >
        {children}
      </div>
    </div>
